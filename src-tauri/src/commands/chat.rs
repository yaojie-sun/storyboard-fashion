...
const SKILL_VERSION_URL: &str = "https://aixiaoxi.top/jy/uploads/install_guide/files/version_fashion.txt";
...
fn get_skill_dir() -> Result<std::path::PathBuf, String> {
    let home = directories::UserDirs::new()
        ...
        .home_dir()
        .to_path_buf();
    Ok(home.join(".claude/skills/xiaoya-ai-cinema-fashion"))
}
...
/// Read local skill version from ~/.claude/skills/xiaoya-ai-cinema-fashion/version.txt
fn read_local_skill_version() -> Result<String, String> {
    let home = directories::UserDirs::new()
        .ok_or("无法解析用户主目录")?
        .home_dir()
        .to_path_buf();
    let version_path = home.join(".claude/skills/xiaoya-ai-cinema-fashion/version.txt");
...
pub(crate) async fn perform_skill_upgrade() -> Result<SkillUpgradeInfo, String> {
    ...
    let target_dir = skill_dir.join("xiaoya-ai-cinema-fashion");

    let zip_url = format!(
        "https://aixiaoxi.top/jy/uploads/install_guide/files/xiaoya-ai-cinema-fashion.zip"
    );
    ...
    if let Ok(content) = tokio::fs::read_to_string(&skill_md_path).await {
        let marker_prefix = "<!-- SECURITY_MARKER: xiaoya-ai-cinema-fashion-protected-skill-v";
        if let Some(start) = content.find(marker_prefix) {
            if let Some(end) = content[start..].find(" -->") {
                let new_marker = format!(
                    "<!-- SECURITY_MARKER: xiaoya-ai-cinema-fashion-protected-skill-v{} -->",
                    server_version
                );
                let updated = format!(
                    "{}{}{}",
